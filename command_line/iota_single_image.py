...
def parse_command_args():
    ...
    parser.add_argument(
        "--backend", type=str, default="dials", help="Backend for processing"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output_dir", type=str, default=None, help="Output directory (for BluIce)"
    )
    parser.add_argument(
        "--termfile", type=str, default=".stop", help="Termination signal filename"
    )
    parser.add_argument(
        "--index", type=int, default=1, help="Numerical index of the image"
    )
    parser.add_argument(
        "--min_bragg",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default="spotfind",
        help="Code for how far to go; available codes: " "spotfind, index, integrate",
    )
    parser.add_argument(
        "--verbose", action="store_true", help="Print information to stdout"
    )
...
class DIALSSpfIdx(Thread):
    def __init__(
        ...
        if self.backend.lower() == "dials":
            if self.paramfile is not None:
                ...
                default_params, _ = write_defaults(
                    method="dials", write_target_file=False, write_param_file=False
                )
                default_phil_string = "\n".join(default_params)
    ...
    def run(self):
        ...
        if self.verbose:
            ...
            print "\n__RESULTS__"
            print "{} {} {} {:.2f} {} {} {} {} {{{}}}".format(
                n_spots, n_overloads, 0, res[1], n_rings, 0, avg_I, 0, err
            )
