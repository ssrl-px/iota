    ...
    def draw_plot(self, min_bragg=0, new_x=None, new_y=None):
        ...
        if nref_x != [] and nref_y != []:
            if self.main_window.tracker_panel.options.chart_window.toggle:
                window_size = (
                    self.main_window.tracker_panel.options.chart_window.window.GetValue()
                )
                if window_size == "":
                    x_min = -1
...
class SpotfinderSettings(wx.Panel):
    def __init__(self, parent):
        ...
        self.chart_window = ct.OptionCtrl(
            self.display,
            items=[("window", "100")],
            checkbox=True,
...
class FileListCtrl(ct.CustomListCtrl):
    ...
    def add_item(self, img, n_obs):
        item = FileListItem(
            path=img, n_obs=n_obs, buttons=ct.MiniButtonBoxInput(self.ctr)
        )
        ...
        # Insert list item
        idx = self.ctr.InsertStringItem(
            self.ctr.GetItemCount() + 1, os.path.basename(item.path)
        )
    ...
    def onMagButton(self, e):
        ...
        path = item_obj.path
        viewer = thr.ImageViewerThread(self, backend="dials", file_string=path)
        viewer.start()
...
class TrackerPanel(wx.Panel):
    def __init__(self, parent):
        ...
        self.status_sizer.Add(self.count_box_sizer, flag=wx.EXPAND | wx.ALL, border=3)
        ...
        self.graph_sizer.Add(self.image_list, 1, flag=wx.EXPAND | wx.LEFT, border=10)
...
class TrackerWindow(wx.Frame):
    def __init__(self, parent, id, title):
        ...
        self.tb_btn_restore = self.toolbar.AddLabelTool(
            wx.ID_ANY,
            label="Restore",
            bitmap=rec_bmp,
            shortHelp="Restore",
        ...
        self.Bind(
            wx.EVT_BUTTON, self.onViewMask, self.tracker_panel.options.mod_mask.btn_mag
        )
        ...
        # Settings bindings
        self.Bind(
            wx.EVT_SPINCTRL, self.onMinBragg, self.tracker_panel.options.min_bragg.ctr
        )
    ...
    def onTimer(self, e):
        """Every second, update spotfinding chart (for some odd reason, chart
        is not updated when the wx.PostEvent happens)"""
        ...
        if self.current_min_bragg != min_bragg:
            ...
            if len(updated_hits) > 0:
                for hit in updated_hits:
                    self.tracker_panel.image_list.image_list.add_item(
                        img=hit[2], n_obs=hit[1]
                    )
    ...
    def plot_results(self):
        ...
        self.tracker_panel.chart.draw_plot(
            min_bragg=min_bragg, new_x=self.new_frames, new_y=self.new_counts
        )
