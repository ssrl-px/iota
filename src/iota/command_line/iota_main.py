...
Description : IOTA main module. Version 1.0.001G
"""
...
import iota.components.iota_gcomps as gcs
...
icons = os.path.join(os.path.dirname(os.path.abspath(gcs.__file__)), "ic/")
...
class ProcThread(Thread):
    """Worker thread; generated so that the GUI is not locked up when
    ...
    def callback(self, input_entry):
        evt = OneDone(tp_EVT_ONEDONE, -1)
        wx.PostEvent(self.parent, evt)
...
class MainWindow(wx.Frame):
    def __init__(self, parent, id, title):
        ...
        # Instantiate windows
        self.input_window = gcs.InputWindow(self)
    ...
    def onWriteImageList(self, e):
        ...
        if img_list_dlg.ShowModal() == wx.ID_OK:
            ...
            self.init_settings()
            self.init.run(
                True, self.gparams, list_flag=True, list_file=img_list_dlg.GetPath()
            )
...
class ProcWindow(wx.Frame):
    def __init__(self, parent, id, title, params):
        ...
        wx.Frame.__init__(
            self, parent, id, title, style=wx.SYSTEM_MENU | wx.CAPTION | wx.CLOSE_BOX
        )
        ...
        # Log output window (WILL BE REPLACED WITH GRAPHS)
        self.log_box = wx.StaticBox(self, label="Log Output", size=(700, 500))
        self.log_sizer = wx.StaticBoxSizer(self.log_box, wx.VERTICAL)
        self.log_window = wx.TextCtrl(
            self,
            style=wx.TE_MULTILINE | wx.TE_READONLY | wx.TE_DONTWRAP,
            size=self.log_box.GetSize(),
        )
        self.log_window.SetFont(wx.Font(8, wx.TELETYPE, wx.NORMAL, wx.NORMAL, False))
        self.log_sizer.Add(
            self.log_window,
        ...
        self.main_sizer.Add(
            self.log_sizer, flag=wx.EXPAND | wx.LEFT | wx.RIGHT | wx.BOTTOM, border=15
        )

        # Processing status bar
        self.sb = self.CreateStatusBar()
        self.sb.SetFieldsCount(2)
        self.sb.SetStatusWidths([200, -2])
        ...
        self.main_sizer.Add(
            self.gauge_process, flag=wx.EXPAND | wx.LEFT | wx.RIGHT, border=10
        )
    ...
    def run(self):
        ...
        if write_def:
            inp.write_defaults(
                self.gparams.output,
                self.txt_out,
                method=self.gparams.advanced.integrate_with,
            )
    ...
    def process_images(self):
        ...
        if self.init.params.image_conversion.convert_only:
            ...
            img_list = [
                [i, len(self.img_objects) + 1, j]
                for i, j in enumerate(self.img_objects, 1)
            ]
    ...
    def onFinishedOneTask(self, e):
        ...
        for root, dirs, files in os.walk(self.check_path):
            for filename in files:
                ...
                if found_file.endswith((self.file_ext)):
                    found_files.append(found_file)
                elif found_file.endswith(".pickle"):
                    processed_images.append(found_file)

        self.gauge_process.Show()
        self.gauge_process.SetValue(len(found_files))

        if os.path.isfile(self.tmp_abort_file):
            self.sb.SetStatusText("Aborting......", 1)
        elif self.stage == "import":
            self.sb.SetStatusText(
                "{} of {} images imported".format(
                    len(found_files), len(self.init.input_list)
                ),
                1,
            )
        elif self.stage == "gs_import":
            self.sb.SetStatusText(
                "{} of {} objects imported".format(
                    len(found_files), len(self.init.gs_img_objects)
                ),
                1,
            )
        elif self.stage == "process":
            self.sb.SetStatusText(
                ...
                "".format(
                    len(found_files), len(self.img_objects), len(processed_images)
                ),
                1,
            )
    ...
    def onFinishedProcess(self, e):
        ...
        if self.stage == "import":
            ...
            if self.process_images():
                self.sb.SetStatusText(
                    "Imported {} images".format(len(self.img_objects)), 1
                )
                self.stage = "process"
            else:
                self.gauge_process.Hide()
                self.proc_toolbar.EnableTool(self.tb_btn_abort.GetId(), False)
                self.sb.SetStatusText(
                    "Converted {} images".format(len(self.img_objects)), 1
                )
                return
        elif self.stage == "process":
            self.img_objects = e.GetValue()
    ...
    def display_log(self):
        ...
        ins_pt = self.log_window.GetInsertionPoint()
        diff = difflib.Differ()
        for i in diff.compare(self.old_logtext, self.logtext):
            if i.startswith("+ "):
                self.log_window.AppendText(i.replace("+", "", 1))
                self.log_window.SetInsertionPoint(ins_pt)


class InputApp(wx.App):
    """App for the input window; will only collect user input and generate
    parameters for the IOTA run; The processing itself will be run in a
    separate ProcessingApp class."""
