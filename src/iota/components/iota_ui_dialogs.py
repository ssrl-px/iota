...
class ImportWindow(BaseDialog):
    ...
    def read_phil(self):
        ...
        self.mod_beamstop.threshold.SetValue(str(self.params.image_conversion.beamstop))
...
class CCTBXOptions(BaseBackendDialog):
    ...
    def set_grid_search(self, idx=1):
        ...
        if idx == 0:
            ...
            self.spot_area.range.SetValue(str(self.params.cctbx.grid_search.area_range))
        elif idx == 2:
            self.signal_search.Enable()
            self.signal_search.SetValue(False)
            self.spot_height.range.Disable()
    ...
    def read_param_phil(self):
        ...
        if self.params.cctbx.selection.prefilter.flag_on:
            ...
            if self.params.cctbx.selection.prefilter.target_unit_cell is not None:
                try:
                    uc = (
                        self.params.cctbx.selection.prefilter.target_unit_cell.parameters()
                    )
                except AttributeError:
                    uc = None
            else:
    ...
    def read_param_phil(self):
        ...
        try:
            if self.params.dials.target_unit_cell is not None:
                uc_str = [
                    str(i) for i in self.params.dials.target_unit_cell.parameters()
                ]
                self.target_uc.cell.SetValue(" ".join(uc_str))
