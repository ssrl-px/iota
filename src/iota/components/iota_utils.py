...
class InputFinder:
    ...
    def get_input(
        self, path, filter_results=True, filter_type="image", last=None, min_back=None
    ):
        ...
        if len(input_list) > 1:
            ...
            if filter_results:
                if filter_type == "self":
                    filter_type = self.get_list_type(file_list=input_list).replace(
                        " folder", ""
                    )
                input_pairs = [
                    i
                    for i in input_pairs
                    if (filter_type in i[1] and not "_tmp" in i[0])
                ]
    ...
    def make_input_list(
        self, paths, filter_results=False, filter_type=None, last=None, min_back=None
    ):
...
class ObjectFinder(object):
    ...
    def find_objects(
        ...
        object_files = ginp.get_file_list(obj_folder, ext_only="int", min_back=min_back)
