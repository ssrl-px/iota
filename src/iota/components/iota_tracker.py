...
class TrackerWindow(wx.Frame):
    ...
    def onView(self, e):
        ...
        viewer = thr.ImageViewerThread(self, backend="dials", file_string=file_string)
        viewer.start()
    ...
    def update_image_list(self):
        ...
        if self.current_min_bragg < min_bragg:
            self.data_dict = {
                idx: item
                for (idx, item) in self.data_dict.iteritems()
                if item[2] > min_bragg
            }
            ...
            putback_hits = [
                ...
                if (i[1] >= min_bragg and i[1] < self.current_min_bragg)
            ]
