...
class ProcThread(Thread):
    ...
    def __init__(
        self, parent, init, iterable, term_file, input_type="image", UI_thread=False
    ):
...
class IOTAUIThread(Thread):
    ...
    def recover_info(self):
        """Recover information from previous run (is here only for backwards
        compatibility reasons; normally all info will come from file)"""
        pass
    ...
    def process_images(self):
        ...
        if self.state == "new images":
            ...
            self.info.msg = "Processing additional {} images ({} total)..." "".format(
                len(iterable), len(self.info.img_list)
            )
        elif self.state == "resume":
            iterable = self.find_images(mode="resume")
            self.info.msg = "Processing {} remaining images ({} total)..." "".format(
                len(iterable), len(self.info.img_list)
            )
        else:
    ...
    def find_images(self, mode="start"):
        ...
        found_files = ginp.make_input_list(
            self.gparams.input, filter=True, filter_type="image", min_back=min_back
        )
        ...
        if mode == "new":
            ...
            old_file_list = [
                obj.raw_img for obj in self.info.image_objects if obj.status == "final"
            ]
            new_file_list = [
                obj.raw_img for obj in self.info.image_objects if obj.status != "final"
            ]
    ...
    def find_objects(self, find_old=False):
        ...
        return obj_finder.find_objects(obj_folder=self.init.obj_base, find_old=find_old)
