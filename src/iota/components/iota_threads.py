...
class ProcThread(Thread):
  """Worker thread; generated so that the GUI does not lock up when processing
  is running."""
  def __init__(self,
               parent,
               init,
...
class ImageViewerThread(Thread):
  """Worker thread that will move the image viewer launch away from the GUI and
  hopefully will prevent the image selection dialog freezing on MacOS."""
  def __init__(self,
               parent,
               file_string,
