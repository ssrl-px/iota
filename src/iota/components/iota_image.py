...
class SingleImage(object):
    ...
    def determine_gs_result_file(self):
        """For 'selection-only' cctbx.xfel runs, determine where the image
        objects are."""
        if self.params.cctbx_ha14.selection.select_only.grid_search_path is not None:
            obj_path = os.path.abspath(
                self.params.cctbx_ha14.selection.select_only.grid_search_path
            )
        else:
            run_number = int(os.path.basename(self.int_base)) - 1
            obj_path = "{}/integration/{:03d}/image_objects" "".format(
    ...
    def import_image(self):
        ...
        if img_data is None:
            self.log_path = util.make_image_path(
                self.raw_img, self.input_base, self.log_base
            )
            self.int_log = os.path.join(
                self.log_path, util.make_filename(self.raw_img) + ".tmp"
            )
            ...
            if not self.params.cctbx_ha14.image_conversion.convert_only:
                ...
                rej_name = (
                    filter(None, util.make_filename(self.conv_img)) + "_reject.int"
                )
        ...
        if (
            self.params.advanced.processing_backend == "cctbx.xfel"
        ...
        if self.params.advanced.processing_backend != "dials" and (
            img_type == "raw"
            or self.params.cctbx_ha14.image_conversion.square_mode != "no_modification"
            ...
            if str(self.params.cctbx_ha14.image_triage.type).lower() in (
                "simple",
                "grid_search",
            ):
                # if self.params.advanced.processing_backend == 'ha14':
