...
class Analyzer(object):
    ...
    def __init__(self, init, all_objects, version, gui_mode=False):
        ...
        self.diff_objects = [i for i in self.all_objects if i.fail != "failed triage"]
    ...
    def print_results(self):
        ...
        final_table.append(
            ...
            "".format(
                np.mean(self.hres), np.std(self.hres), max(self.hres), min(self.hres)
            )
        )
        final_table.append(
    ...
    def print_summary(self, write_files=True):
        ...
        if write_files:
            ...
            if (
                self.params.advanced.integrate_with == "cctbx"
                and len(self.filter_fail_objects) > 0
            ):
