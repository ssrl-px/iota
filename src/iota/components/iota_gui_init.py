...
class MainWindow(wx.Frame):
    ...
    def __init__(self, parent, id, title):
        ...
        self.tb_btn_test = self.toolbar.AddLabelTool(
            wx.ID_ANY, label="Test", bitmap=test_bmp
        )
    ...
    def init_settings(self):
        ...
        self.gparams.advanced.integrate_with = str(
            self.input_window.int_box.ctr.GetString(
                self.input_window.int_box.ctr.GetSelection()
            )
        ).lower()
    ...
    def onRecovery(self, e):
        ...
        if path_dlg.ShowModal() == wx.ID_OK:
            ...
            for inp in self.gparams.input:
                if not os.path.isdir(inp):
                    ...
                    if error_msg.ShowModal() == wx.ID_YES:
                        open_dlg = wx.DirDialog(
                            self, "Choose the input folder:", style=wx.DD_DEFAULT_STYLE
                        )
                        if open_dlg.ShowModal() == wx.ID_OK:
            ...
            if not os.path.isdir(self.gparams.output):
                ...
                if error_msg.ShowModal() == wx.ID_YES:
                    open_dlg = wx.DirDialog(
                        self, "Choose the output folder:", style=wx.DD_DEFAULT_STYLE
                    )
                    if open_dlg.ShowModal() == wx.ID_OK:
                        ...
                        rec_init.conv_base = os.path.join(
                            self.gparams.output, "converted_pickles", cnv_no
                        )
                        rec_init.int_base = os.path.join(
                            self.gparams.output, "integration", int_no
                        )
                        rec_init.fin_base = os.path.join(rec_init.int_base, "final")
                        rec_init.log_base = os.path.join(rec_init.int_base, "logs")
                        rec_init.obj_base = os.path.join(
                            rec_init.int_base, "image_objects"
                        )
                        rec_init.viz_base = os.path.join(
                            rec_init.int_base, "visualization"
                        )
    ...
    def fix_old_phil(self, phil):
        ...
        if not hasattr(params.image_conversion, "rename_pickle"):
            ...
            if "none" in str(prefix).lower():
                params.image_conversion.__inject__(
