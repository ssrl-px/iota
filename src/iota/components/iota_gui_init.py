...
class MainWindow(wx.Frame):
    ...
    def __init__(self, parent, id, title):
        ...
        # Single input window
        main_box.Add(
            self.input_window, flag=wx.LEFT | wx.RIGHT | wx.TOP | wx.BOTTOM, border=10
        )
        main_box.Add((-1, 20))
    ...
    def init_settings(self):
        ...
        self.gparams.advanced.integrate_with = str(
            self.input_window.int_cbx_choice.GetString(int_index)[:5]
        ).lower()
...
class ProcWindow(wx.Frame):
    ...
    def plot_integration(self):
        try:
            ...
            self.chart_tab.nsref_axes.scatter(
                self.nref_xaxis, self.nref_list, marker="o", color="red", picker=True
            )
            self.chart_tab.nsref_axes.set_xlim(0, np.max(self.nref_xaxis) + 1)
            self.chart_tab.nsref_axes.set_ylim(ymin=0)
            self.chart_tab.nsref_axes.set_ylabel(
                "Ref. (I / sigI > {})" "".format(self.gparams.cctbx.selection.min_sigma)
            )

            # Resolution per frame
            self.chart_tab.res_axes.clear()
            self.chart_tab.res_axes.grid(True)
            self.chart_tab.res_axes.scatter(
                self.nref_xaxis, self.res_list, marker="d", color="green", picker=True
            )
...
class InputWindow(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        self.int_txt_choice = wx.StaticText(
            self, label="Integrate with: ", size=(120, -1)
        )
        ...
        self.inp_btn_mag = wx.BitmapButton(
            self, bitmap=wx.Bitmap("{}/16x16/viewmag.png" "".format(icons))
        )
        ctr_length = (
            main_box.GetSize()[0]
            - self.inp_txt.GetSize()[0]
            - self.inp_btn_browse.GetSize()[0]
            - self.inp_btn_mag.GetSize()[0]
            - 55
        )
        ...
        self.out_btn_mag = wx.BitmapButton(
            self, bitmap=wx.Bitmap("{}/16x16/viewmag.png" "".format(icons))
        )
