        ...
        if sum(self.nref_list) > 0 and sum(self.res_list) > 0:
            try:
                ...
                self.nsref_x = np.array(
                    [i + 1 for i in range(len(self.img_list))]
                ).astype(np.double)
...
class ProcWindow(wx.Frame):
    ...
    def recover(self, int_path, status, init, params):
        ...
        object_finder = thr.ObjectFinderThread(
            self, object_folder=self.init.obj_base, new_fin_base=init.fin_base
        )
        object_finder.start()
            ...
            if not self.gparams.image_conversion.convert_only:
                ...
                if clusters != []:
                    for cluster in clusters:
                        ...
                        fname = wx.StaticText(
                            self.summary_tab.cluster_panel,
    ...
    def onTimer(self, e):
        ...
        if self.start_object_finder:
            ...
            object_finder = thr.ObjectFinderThread(
                self, last_object=last_object, object_folder=self.init.obj_base
            )
            object_finder.start()
