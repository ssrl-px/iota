...
class Integrator:
    """Class for indexing, integration, etc.

    using current cctbx.xfel
    """
    ...
    def find_spots(self):
        # Perform spotfinding
        self.observed = self.processor.find_spots(datablock=self.img_object.datablock)
            ...
            if self.img_object.fail is None:
                try:
                    ...
                    if hasattr(e, "classname"):
                        ...
                        print(
                            "Integration error for {}:".format(self.img_object.img_path)
                        )
                        error_message = "{}".format(str(e).replace("\n", " ")[:50])
                    print(error_message)
