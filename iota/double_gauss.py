...
def double_gauss_spotfinder(img_data, g1=2, g2=10, thresh=1.5):
    """spotfinder prototype using gaussian local background estimation, and
    gaussian smoothing.
...
class SpotManager:
    ...
    def update_smoothing(self, smooth_thresh):
        ...
        self.smooth_img = ndi.filters.gaussian_filter(self.img_data, self.smooth_thresh)
    ...
    def plot_spots(self):
        """Create an interactive plot using the data produced by
        `double_gauss_spotfinder`"""
        ...
        def update_thresh(val):
            ...
            ax2.scatter(
                spots[1], spots[0], s=10, c="r", marker="o", linewidths=0, alpha=1
            )
            plt.draw()
        ...
        def write_params(event):
            print "Background smoothing: {:.2f}\nSpot smoothing: {:.2f}\nSpot threshold: {:.2f}".format(
                self.bg_thresh, self.smooth_thresh, self.spot_thresh
            )
...
def plot_spots_simple(
    img_data, blured_img, background, init_thresh=1.5, thresh_range=(1, 4)
):
    """Create an interactive plot using the data produced by
    `double_gauss_spotfinder`"""
    ...
    sthresh = Slider(
        axthresh, "Threshold", thresh_range[0], thresh_range[1], valinit=init_thresh
    )
...
def analyze_threshold(img, bg):
    ...
    n_spots = [
        len(get_spots(img, bg, thresh)[0]) / (len(img) ** 2) for thresh in thresholds
    ]
