    ...
    if gs_params.selection.prefilter.flag_on:
        for i in int_list:
            try:
                ...
                if (
                    gs_params.selection.prefilter.target_pointgroup != None
                    and user_uc != None
                ):
                    if (
                        p_pg
                        == gs_params.selection.prefilter.target_pointgroup.replace(
                            " ", ""
                        )
                        and uc_check
                    ):
    ...
    if not os.path.isfile(result_file):
        ...
        if len(acceptable_results) == 0:
            ...
            avg_cell = (
                "{:>8.2f}, {:>8.2f}, {:>8.2f}, {:>6.2f}, {:>6.2f}, {:>6.2f}"
                "".format(
                    np.mean([item["a"] for item in acceptable_results]),
            ...
            std_cell = (
                "{:>8.2f}, {:>8.2f}, {:>8.2f}, {:>6.2f}, {:>6.2f}, {:>6.2f}"
                "".format(
                    np.std([item["a"] for item in acceptable_results]),
