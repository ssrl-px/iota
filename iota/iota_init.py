...
class InitAll(object):
    ...
    def analyze_prior_results(self, analysis_source):
        """Runs analysis of previous grid search / integration results, used in
        an analyze-only mode."""
        ...
        if os.path.isdir(analysis_source):
            ...
            try:
                int_folder = os.path.abspath(
                    os.path.join(
                        os.curdir, "integration/{}/grid_search".format(analysis_source)
                    )
                )
            except ValueError:
        ...
        if os.path.isdir(int_folder):
            ...
            analysis.unit_cell_analysis(
                self.params.analysis.cluster_threshold, int_folder, False
            )
    ...
    def run(self):
        ...
        if self.args.path == None:  # No input
            ...
            if os.path.exists(carg):
                ...
                elif os.path.isfile(carg) and not os.path.basename(carg).endswith(
                    ".lst"
                ):
        ...
        if (
            ...
            or self.params.analysis.charts
        ):
