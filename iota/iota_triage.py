...
def spotfinding_param_search(input_img, gs_params):
    ...
    with Capturing() as junk_output:
        Org = DistlOrganizer(verbose=False, argument_module=E, phil_params=params)
    ...
    if Bragg_spots >= gs_params.image_triage.min_Bragg_peaks:
        inp.main_log(
            logfile,
            "{} --> ACCEPTED! {} good Bragg peaks" "".format(input_img, Bragg_spots),
        )
        return input_img
    else:
...
if __name__ == "__main__":
    ...
    gs_params = inp.auto_mode(
        os.path.abspath(os.path.curdir), os.path.abspath(abs_inp_path), now
    )[0]
    ...
    accepted_img = parallel_map(
        iterable=file_list, func=triage_mproc_wrapper, processes=32, preserve_order=True
    )
