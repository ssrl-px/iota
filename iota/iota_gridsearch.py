...
def integrate_one_image(mp_entry, n_int, log_dir, save_pickle, gs_params):
    ...
    result_file = os.path.normpath(
        "{0}/int_{1}.lst" "".format(current_output_dir, img_no_ext)
    )
    ...
    with Capturing() as index_log:
        try:
            ...
            int_results = info.organizer.info["best_integration"]["integration"][
                "AD14_parameters"
            ]
    ...
    if int_results != None:
        ...
        cell = "{:>6.2f}, {:>6.2f}, {:>6.2f}, {:>6.2f}, {:>6.2f}, {:>6.2f}" "".format(
            float(uc[0]),
            float(uc[1]),
            float(uc[2]),
            float(uc[3]),
        ...
        int_status = "RES: {:>4.2f} SG: {:^{wsg}}  CELL: {}" "".format(
            init_res, sg, cell, wsg=len(str(sg))
        )
    else:
...
def final_integrate_one(mp_entry, log_dir, n_int, gs_params):
    ...
    outputting a pickle file, but a list of spotfinding parameters with
    ...
    current_error_file = os.path.normpath(
        "{0}/err_{1}.log".format(log_dir, img_filename.split(".")[0])
    )
    ...
    if gs_params.advanced.charts:
        advanced_args = [
            ...
            "integration.pdf_output_dir={0}/pdf_h{1}_a{2}".format(
                current_output_dir, spot_height, spot_area
            ),
        ]
        if not os.path.exists(
            "{0}/pdf_h{1}_a{2}".format(current_output_dir, spot_height, spot_area)
        ):
            os.makedirs(
                "{0}/pdf_h{1}_a{2}".format(current_output_dir, spot_height, spot_area)
            )
    ...
    # for mosaicity chart only
        ...
        advanced_args = [
            "integration.enable_residual_map=False",
            "integration.enable_residual_map_deltapsi=True",
    ...
    if int_results != None:
        ...
        int_status = (
            ...
            "".format(resol, strong_spots, dom_size, mosaicity, mos_quality, pos_rmsd)
        )
...
def int_single_image(mp_entry, log_dir, n_int, gs_params):
    """Performs grid search of spotfinding parameters. Set aside for single-
    image mode ONLY. Need to clean this up later such that have only one
    integration module.
...
def exp_integrate_one(mp_entry, log_dir, n_int, gs_params):
    ...
    if int_results != None:
        ...
        for i in sorted(int_final.keys()):
            print i
        # observations = [a.get_obs(int_final['spacegroup']) for a in int_final['results']]
        # print len(observations)

        results_one_image = info.organizer.info["best_integration"]["integration"][
            "results"
        ][0]
        spacegroup = info.organizer.info["best_integration"]["integration"][
            "spacegroup"
        ]
