...
def integrate_image(mp_entry, current_log_file, arguments, ptitle, n_int, gs_params):
    ...
    if int_AD14 != None:
        ...
        # Extract relevant information from integration results ("info" object)
        ...
        mosaicity = int_AD14["fw_mos_deg"]
        mos_quality = int_AD14["mosaic_model_area_under_green_curve_sampled"]
        ...
        p_cell = "{:>6.2f}, {:>6.2f}, {:>6.2f}, {:>6.2f}, {:>6.2f}, {:>6.2f}" "".format(
            cell[0], cell[1], cell[2], cell[3], cell[4], cell[5]
        )

        int_status = "RES: {:>4.2f} SG: {:^{wsg}}  CELL: {}" "".format(
            res, sg, p_cell, wsg=len(str(sg))
        )
    else:
...
def gs_integration(mp_entry, n_int, log_dir, gs_params):
    ...
    advanced_args = organize_parameters(
        current_output_dir, spot_height, spot_area, gs_params
    )
    ...
    results, int_status = integrate_image(
        mp_entry, current_log_file, arguments, "GRID SEARCH", n_int, gs_params
    )
...
def final_integration(mp_entry, n_int, log_dir, gs_params):
    ...
    # run integration
    results, int_status = integrate_image(
        mp_entry, current_log_file, arguments, "INTEGRATING", n_int, gs_params
    )
