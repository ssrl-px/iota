...
class Integrator(object):
    ...
    def __init__(self, source_image=None, object_folder=None, gain=0.32, params=None):
        ...
        self.parser = OptionParser(
            phil=current_phil, read_datablocks=True, read_datablocks_from_images=True
        )
            ...
            for e_number in xrange(len(self.experiments)):
                ...
                try:
                    ...
                    event_timestamp = os.path.splitext(
                        self.experiments[0].imageset.paths()[0]
                    )[0]
                ...
                if hasattr(self.phil.output, "output_dir"):
                    outfile = os.path.join(
                        self.phil.output.output_dir,
                        self.phil.output.integration_pickle
                        % (event_timestamp, e_number),
                    )
                else:
                    outfile = os.path.join(
                        os.path.dirname(self.phil.output.integration_pickle),
                        self.phil.output.integration_pickle
                        % (event_timestamp, e_number),
                    )
    ...
    def run(self):
        ...
        with open("test.txt", "w") as tf:
            ...
            tf.write(
                "\n\n{} {} {}".format(
                    len(self.observed), len(self.indexed), len(self.integrated)
                )
            )
