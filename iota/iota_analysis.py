...
class Analyzer(object):
    ...
    def __init__(self, all_objects, logfile, version, now):
        ...
        self.final_objects = [
            ...
            if i.triage == "accepted"
            and i.prefilter == True
            and i.final["final"] != None
        ]
    ...
    def print_results(self):
        ...
        final_table.append(
            ...
            "".format(np.mean(self.s), np.std(self.s), max(self.s), min(self.s), cons_s)
        )
        final_table.append(
            "Avg. spot height:      {:<8.3f}  std. dev:    {:<6.2f}"
            "  max: {:<3}  min: {:<3}  consensus: {:<3}"
            "".format(np.mean(self.h), np.std(self.h), max(self.h), min(self.h), cons_h)
        )
        final_table.append(
            "Avg. spot areas:       {:<8.3f}  std. dev:    {:<6.2f}"
            "  max: {:<3}  min: {:<3}  consensus: {:<3}"
            "".format(np.mean(self.a), np.std(self.a), max(self.a), min(self.a), cons_a)
        )
        final_table.append(
        ...
        if len(self.final_objects) == 1:
            ...
            for cluster in clusters:
                ...
                output_file = os.path.join(
                    output_dir, "uc_cluster_{}.lst".format(counter)
                )
    ...
    def print_summary(self, int_base):
        ...
        summary.append(
            "raw images not integrated:           {}" "".format(len(not_int_objects))
        )
