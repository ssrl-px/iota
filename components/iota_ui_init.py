...
class MainWindow(IOTABaseFrame):
    ...
    def on_startup(self):
        ...
        if paramfile:
            self.load_script(filepath=paramfile, reset=False, update_input_window=False)
        else:
            self.iota_phil = inp.process_ui_input(
                args=self.args, phil_args=self.phil_args, input_source=None
            )
        ...
        if messages:
            ...
            wx.MessageBox(
                caption="Command-line Argument Error!",
                message=msg_string,
    ...
    def onImportOptions(self, e):
        ...
        if self.gparams.advanced.processing_backend == "cctbx.xfel":
            ...
            imp_dialog = dlg.HA14ImportWindow(
                self,
                phil=self.iota_phil,
                title="Import Options",
                style=wx.DEFAULT_DIALOG_STYLE | wx.STAY_ON_TOP,
    ...
    def onProcessOptions(self, e):
        ...
        # For deprecated cctbx.xfel HA14 options
            ...
            int_dialog = dlg.HA14BackendOptions(
                self,
                phil=self.iota_phil,
                target=self.target_phil,
    ...
    def onAnalysisOptions(self, e):
        ...
        if self.gparams.advanced.processing_backend == "cctbx.xfel":
            ...
            an_dialog = dlg.HA14AnalysisWindow(
                self,
                phil=self.iota_phil,
                title="Dataset Analysis Options",
                style=wx.DEFAULT_DIALOG_STYLE | wx.STAY_ON_TOP | wx.RESIZE_BORDER,
    ...
    def onRun(self, e):
        ...
        self.proc_window = frm.ProcWindow(
            self, -1, title="", target_phil=self.target_phil, phil=self.iota_phil
        )
        init = UIInitAll()
    ...
    def onOutputScript(self, e):
        ...
        if save_dlg.ShowModal() == wx.ID_OK:
            ...
            if self.target_phil is not None:
                fn = "{}.phil".format(
                    self.gparams.advanced.processing_backend.replace(".", "_")
                )
    ...
    def load_script(self, filepath, reset=True, update_input_window=True):
        ...
        self.iota_phil = inp.process_ui_input(
            input_source=filepath, args=self.args, phil_args=self.phil_args, mode="file"
        )
