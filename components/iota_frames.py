...
class ProcessingTab(wx.Panel):
    ...
    def draw_plots(self, finished_objects, img_list, nref_list, res_list):
        try:
            ...
            categories = [
                ...
                [
                    ...
                    "#f46d43",
                    len(
                        [i for i in finished_objects if i.fail == "failed grid search"]
                    ),
                ],
                [
                    "failed filter",
                ...
                [
                    ...
                    "#f46d43",
                    len(
                        [i for i in finished_objects if i.fail == "failed spotfinding"]
                    ),
                ],
                [
                    "failed indexing",
                ...
                [
                    ...
                    "#fee090",
                    len(
                        [i for i in finished_objects if i.fail == "failed integration"]
                    ),
                ],
                [
                    "integrated",
        ...
        if sum(nref_list) > 0 and sum(res_list) > 0:
            try:
                ...
                nsref_x = np.array([i + 1.5 for i in range(len(img_list))]).astype(
                    np.double
                )
                nsref_y = np.array([np.nan if i == 0 else i for i in nref_list]).astype(
                    np.double
                )
                nsref_ylabel = "Reflections (I/{0}(I) > {1})" "".format(
                    r"$\sigma$", self.gparams.cctbx.selection.min_sigma
                )
                nsref = self.nsref_axes.scatter(
                    nsref_x,
                    nsref_y,
                    s=45,
                    marker="o",
                ...
                self.res_axes.clear()
                res_x = np.array([i + 1.5 for i in range(len(img_list))]).astype(
                    np.double
                )
                res_y = np.array([np.nan if i == 0 else i for i in res_list]).astype(
                    np.double
                )
                res_m = np.isfinite(res_y)

                res = self.res_axes.scatter(
                    res_x[res_m],
                    res_y[res_m],
                    s=45,
                    marker="o",
...
class ProcWindow(wx.Frame):
    ...
    def __init__(self, parent, id, title, phil, target_phil=None, recover=None):
        wx.Frame.__init__(
            self,
            parent,
            id,
    ...
    def plot_integration(self):
        ...
        self.chart_tab.draw_plots(
            finished_objects=self.finished_objects,
