...
class FileListCtrl(ct.CustomListCtrl):
    ...
    def view_images(self, img_list):
        ...
        backend = self.parent.int_box.ctr.GetString(
            self.parent.int_box.ctr.GetSelection()
        ).lower()
        viewer = thr.ImageViewerThread(self, backend=backend, file_string=file_string)
        viewer.start()
...
class ProcessingTab(wx.Panel):
    def __init__(self, parent):
        ...
        int_gsub = gridspec.GridSpecFromSubplotSpec(
            2, 1, subplot_spec=int_gsp[:2, :], hspace=0
        )
        ...
        uc_gsub = gridspec.GridSpecFromSubplotSpec(
            2, 3, subplot_spec=int_gsp[2:4, :3], hspace=0, wspace=0
        )
        ...
        self.beta_axes = self.int_figure.add_subplot(uc_gsub[4], sharey=self.alpha_axes)
        ...
        self.gamma_axes = self.int_figure.add_subplot(
            uc_gsub[5], sharey=self.alpha_axes
        )
        ...
        if sum(self.nref_list) > 0 and sum(self.res_list) > 0:
            try:
                ...
                self.nsref_x = np.array(
                    [i + 1.5 for i in range(len(self.img_list))]
                ).astype(np.double)
                    ...
                    if axis == "nsref":
                        if not np.isnan(self.nsref_y[idx]):
                            ...
                            self.nsref_pick.set_data(
                                self.nsref_x[idx], self.nsref_y[idx]
                            )
                    elif axis == "res":
    ...
    def onImageView(self, e):
        ...
        if os.path.isfile(filepath):
            viewer = thr.ImageViewerThread(self, backend=backend, file_string=filepath)
            viewer.start()
...
class SummaryTab(wx.Panel):
    def __init__(
        ...
        if self.gparams.analysis.run_clustering:
            ...
            self.cluster_box_grid.Add(
                wx.StaticText(self.cluster_panel, label="No. Images"), pos=(0, 0)
            )
            self.cluster_box_grid.Add(
                wx.StaticText(self.cluster_panel, label="Bravais Lattice"), pos=(0, 1)
            )
            self.cluster_box_grid.Add(
                wx.StaticText(self.cluster_panel, label="Unit Cell"), pos=(0, 2)
            )
            self.cluster_box_grid.Add(
                wx.StaticText(self.cluster_panel, label="Filename"), pos=(0, 3)
            )
            self.cluster_box_grid.AddGrowableCol(2)
            self.cluster_panel.SetFont(sfont)

            # Insert into sizers
            cluster_box_sizer.Add(
                self.cluster_panel, flag=wx.EXPAND | wx.ALL, border=10
            )
        ...
        if len(self.final_objects) == 0:
            ...
            if clusters != []:
                for cluster in clusters:
                    ...
                    n_ucs = wx.StaticText(
                        self.summary_tab.cluster_panel, label=str(cluster["number"])
                    )
                    pg_info = wx.StaticText(
                        self.summary_tab.cluster_panel, label=str(cluster["pg"])
                    )
                    uc_info = wx.StaticText(
                        self.summary_tab.cluster_panel, label=str(cluster["uc"])
                    )
                    fname = wx.StaticText(
                        self.summary_tab.cluster_panel, label=str(cluster["filename"])
                    )
    ...
    def onTimer(self, e):
        ...
        if self.start_object_finder:
            ...
            object_finder = thr.ObjectFinderThread(
                self, object_folder=self.init.obj_base
            )
            object_finder.start()
