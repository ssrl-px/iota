...
class XInitAll(InitBase):
    ...
    def initialize_interface(self):
        ...
        if not self.args.path:  # No input
            ...
            self.iota_phil = inp.process_input(
            ...
            # If user provided gibberish
                ...
                msg = (
                    "IOTA_XTERM_INIT_ERROR: Invalid input! Need parameter filename "
                    "or data folder."
                )
                print(self.logo)
                print(msg)
            ...
            self.iota_phil = inp.process_input(
                self.args, self.phil_args, carg, mode, self.now
            )
            self.params = self.iota_phil.extract()
    ...
    def initialize_info_object(self):
        ...
        self.info.img_list = [
            [i, len(self.input_list) + 1, j] for i, j in enumerate(self.input_list, 1)
        ]
