...
class ProcThread(Thread):
    ...
    def __init__(self, parent, init, iterable, term_file, input_type="image"):
        ...
        if self.init.params.mp_method == "multiprocessing":
            ...
            try:
                if self.init.params.mp_method == "lsf":
                    command = (
                        "bsub -q {} -n {} iota.process {} --files {} --type {} "
                        "--stopfile {}".format(
                            queue,
                            nproc,
                            init_path,
...
class ImageFinderThread(Thread):
    ...
    def __init__(self, parent, image_paths, image_list):
        Thread.__init__(self)
        self.parent = parent
...
class ObjectFinderThread(Thread):
    ...
    def __init__(self, parent, object_folder, fix_paths=False, new_fin_base=None):
        Thread.__init__(self)
        self.parent = parent
...
class ImageViewerThread(Thread):
    ...
    def __init__(self, parent, file_string, viewer="dials.image_viewer", img_type=None):
        Thread.__init__(self)
        self.parent = parent
...
class SpotFinderThread(Thread):
    ...
    def __init__(self, parent, data_list=None, term_file=None, processor=None):
        Thread.__init__(self)
        self.parent = parent
