...
class JobSubmitThread(Thread):
    ...
    def __init__(self, parent, params, output_folder, abort_file):
        ...
        if params.mp.method == "lsf":
            ...
            self.command = (
                ...
                "".format(
                    logfile, queue, nproc, self.job_id, init_path, iter_path, abort_file
                )
            )
        elif params.mp.method == "torq":
            sub_string = "{} --files {} --stopfile {} --mode ui" "".format(
    ...
    def run(self):
        if self.command is not None:
            print(self.command)
            self.job = CustomRun(command=self.command, join_stdout_stderr=True)
            self.job.run()
...
class ObjectCruncher:
    ...
    def objects_from_folder(self):
        self.info.unread_files = ginp.get_file_list(self.object_folder, ext_only="int")

    def read_object(self, filepath):
        """Reads (using easy_pickle) an object pickle and extracts relevant
        information; this will be put into the various lists and dictionaries
        in the info object; the full image-associated object will also be
        saved.
    ...
    def get_new_finished_objects(self):
        ...
        if read_files:
            self.info.unread_files = [
                f for f in self.info.unread_files if f not in read_files
            ]
            ...
            try:
                if not obj.fail:
                    ...
                    try:
                        ...
                        print(
                            "CLUSTER_ITERABLE_ERROR for {}: {}".format(obj.obj_file, e)
                        )
                        pass
    ...
    def calculate_summary_chart(self):
        ...
        failed_idx_int = len(
            [i for i in self.info.finished_objects if i.fail == "failed grid search"]
        )
        failed_filter = len(
            [
                i
        ...
        failed_spf = len(
            [i for i in self.info.finished_objects if i.fail == "failed spotfinding"]
        )
        failed_idx = len(
            [i for i in self.info.finished_objects if i.fail == "failed indexing"]
        )
        failed_int = len(
            [i for i in self.info.finished_objects if i.fail == "failed integration"]
        )
        integrated = len(
            [
                i
    ...
    def calculate_proc_plot(self):
        self.info.nsref_x = np.array(
            [i + 1 for i in range(len(self.info.img_list))]
        ).astype(np.double)
        self.info.nsref_y = np.array(
            [np.nan if i == 0 else i for i in self.info.nref_list]
        ).astype(np.double)
        self.info.nsref_median = np.median([i for i in self.info.nref_list if i > 0])

        self.info.res_x = np.array(
            [i + 1 for i in range(len(self.info.img_list))]
        ).astype(np.double)
        self.info.res_y = np.array(
            [np.nan if i == 0 else i for i in self.info.res_list]
        ).astype(np.double)
...
class ObjectReaderThread(Thread):
    ...
    def run(self):
        self.obj_worker = ObjectCruncher(
            info=self.info, object_folder=self.object_folder
        )
...
class ObjectFinderThread(Thread):
    ...
    def run(self):
        if self.last_object is not None:
            ...
            last = None
        object_files = ginp.get_file_list(self.object_folder, ext_only="int", last=last)
...
class PRIMEThread(Thread):
    ...
    def __init__(self, parent, init, info=None, best_pg=None, best_uc=None):
    ...
    def prepare_PRIME_input(self):
        ...
        if self.best_pg and self.best_uc:
            ...
            analyzer = Analyzer(init=self.init, all_objects=self.info.finished_objects)
            ...
            # Queue options
            if (
                self.init.params.mp.method == "lsf"
                and self.init.params.mp.queue is not None
            ):
