...
class Plotter(object):
    def __init__(self, params, final_objects, viz_dir=None):
    ...
    def plot_res_histogram(self, write_files=False):
        ...
        hr_n, hr_bins, hr_patches = plt.hist(
            hres, 20, normed=False, facecolor="b", alpha=0.75, histtype="stepfilled"
        )
        ...
        lr_n, lr_bins, lr_patches = plt.hist(
            lres, 20, normed=False, facecolor="b", alpha=0.75, histtype="stepfilled"
        )
...
class Analyzer(object):
    ...
    def __init__(self, init, all_objects, gui_mode=False):
        ...
        if self.final_objects is not None:
            self.sorted_final_images = sorted(
                self.final_objects, key=lambda i: i.final["mos"]
            )
            self.no_diff_objects = [
            ...
            if self.params.advanced.integrate_with == "cctbx":
                ...
                self.not_idx_objects = [
                    i for i in self.all_objects if i.fail == "failed indexing"
                ]
                self.filter_fail_objects = [
                    i for i in self.all_objects if i.fail == "failed filter"
                ]
                self.not_int_objects = [
            ...
            self.analysis_result.__setattr__(
                "filter_fail_objects", len(self.filter_fail_objects)
            )
            self.analysis_result.__setattr__("final_objects", len(self.final_objects))
            self.analysis_result.__setattr__(
                "no_diff_objects", len(self.no_diff_objects)
            )
            self.analysis_result.__setattr__(
                "not_int_objects", len(self.not_int_objects)
            )
            if self.params.advanced.integrate_with == "dials":
                self.analysis_result.__setattr__(
                    "not_spf_objects", len(self.not_spf_objects)
                )
                self.analysis_result.__setattr__(
                    "not_idx_objects", len(self.not_idx_objects)
                )
    ...
    def print_summary(self, write_files=True):
        ...
        misc.main_log(
            self.logfile, "\n\n{:-^80}\n".format("SUMMARY"), (not self.gui_mode)
        )
        ...
        summary.append(
            "raw images with no diffraction:      {}"
        ...
        if self.params.advanced.integrate_with == "cctbx":
            ...
            summary.append("failed filter:" "".format(len(self.filter_fail_objects)))
            summary.append(
                "failed integration:                  {}"
                "".format(len(self.not_int_objects))
            )
        summary.append(
            "final integrated pickles:            {}"
    ...
    def make_prime_input(self, filename="prime.phil"):
        ...
        prime_params.run_no = os.path.join(
            os.path.dirname(self.prime_data_path), "prime/001"
        )
