    ...
    def __init__(self, init, all_objects, gui_mode=False):
        ...
        if self.final_objects is not None:
            ...
            self.analysis_result.__setattr__(
                "n_filter_fail_objects", len(self.filter_fail_objects)
            )
            self.analysis_result.__setattr__("n_final_objects", len(self.final_objects))
            self.analysis_result.__setattr__(
                "n_no_diff_objects", len(self.no_diff_objects)
            )
            self.analysis_result.__setattr__(
            ...
            if self.params.advanced.integrate_with == "dials":
                ...
                self.analysis_result.__setattr__(
                    "n_not_idx_objects", len(self.not_idx_objects)
                )
