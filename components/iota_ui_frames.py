...
class ProcessingTab(wx.Panel):
        ...
        try:
            ...
            if self.pick["picked"] and self.pick["axis"] == "summary":
                ...
                if cat not in ("integrated", "not processed"):
                    self.proc_fnames = [
                        i.img_path for i in self.info.finished_objects if i.fail == cat
                    ]
                elif cat == "integrated":
                    self.proc_fnames = [
                        i.img_path
                        for i in self.info.finished_objects
                        if i.fail is None and i.final["final"] is not None
                    ]
                elif cat == "not processed":
                    self.proc_fnames = [
                        i.img_path
                        for i in self.info.finished_objects
                        if i.fail is None and i.final["final"] is None
                    ]
                else:
                    self.proc_fnames = ""
    ...
    def draw_integration_plots(self):
        try:
            ...
            nsref_ylabel = "Reflections (I/{0}(I) > {1})" "".format(
                r"$\sigma$", self.gparams.image_import.strong_sigma
            )
            ...
            nsref_med = self.nsref_axes.axhline(
                self.info.nsref_median, c="#ca0020", ls="--"
            )
        ...
        try:
            ...
            with warnings.catch_warnings():
                ...
                res_med = self.res_axes.axhline(
                    self.info.res_median, c="#0571b0", ls="--"
                )
            ...
            if self.pick["picked"] and self.pick["axis"] != "summary":
                ...
                if axis == "nsref":
                    if not np.isnan(self.info.nsref_y[idx]):
                        ...
                        self.nsref_pick.set_data(
                            self.info.nsref_x[idx], self.info.nsref_y[idx]
                        )
                elif axis == "res":
                    if not np.isnan(self.info.res_y[idx]):
                        self.res_pick.set_visible(True)
                        self.res_pick.set_data(
                            self.info.res_x[idx], self.info.res_y[idx]
                        )
    ...
    def draw_measured_indices(self):
        ...
        slice = self.info.merged_indices.slice(
...
class LiveAnalysisTab(d.ScrolledPanel):
    def __init__(self, parent, init=None, gparams=None, finished_objects=None):
        self.parent = parent
        self.init = init
...
class SummaryTab(d.ScrolledPanel):
    def __init__(
        self, parent, init=None, gparams=None, final_objects=None, out_dir=None
    ):
        d.ScrolledPanel.__init__(self, parent)
    ...
    def initialize_standalone_plot(self, figsize=(8, 8)):
        ...
        self.plot = Plotter(
            self.plot_window, params=self.gparams, final_objects=self.final_objects
        )
...
class ProcWindow(IOTABaseFrame):
    ...
    def __init__(self, parent, id, title, phil, target_phil=None):
        ...
        self.Bind(wx.aui.EVT_AUINOTEBOOK_PAGE_CHANGED, self.onTabChange, self.proc_nb)
    ...
    def set_proc_iterable(self):
        ...
        if self.state == "new images":
            ...
            msg = "Processing additional {} images ({} total)..." "".format(
                len(self.new_images), len(self.info.img_list)
            )
            return self.new_images, msg

        elif self.state == "resume":
            subset = (
                self.gparams.advanced.image_range.flag_on
                or self.gparams.advanced.random_sample.flag_on
            )
            ...
            if iterable:
                msg = "Processing {} remaining images ({} total)..." "".format(
    ...
    def process_images(self):
        ...
        self.proc_thread = thr.JobSubmitThread(
            self,
            params=self.gparams,
    ...
    def analyze_results(self, analysis=None):
        if len(self.final_objects) == 0:
            ...
            if analysis is None:
                self.recovery = False
                analysis = Analyzer(
                    self.init, self.info.finished_objects, gui_mode=True
                )
    ...
    def run_prime_thread(self):
        ...
        self.prime_thread = thr.PRIMEThread(
            self, info=self.info, init=self.init, best_pg=pg, best_uc=uc
        )
    ...
    def onProcTimer(self, e):
        ...
        if self.info.unprocessed_images <= 0:
            ...
            if not (hasattr(self, "object_reader") and self.object_reader.is_alive()):
                ...
                self.object_reader = thr.ObjectReaderThread(
                    self, info=self.info, source=self.info.obj_list_file
                )
    ...
    def get_images_from_filesystem(self):
        img_finder = thr.ImageFinderThread(
            self, image_paths=self.gparams.input, image_list=self.info.img_list
        )
        img_finder.start()
    ...
    def finish_process(self):
        ...
        if self.run_aborted:
            ...
            if not self.info.finished_objects:  # Find finished objects if none loaded
                ...
                self.object_reader = thr.ObjectReaderThread(
                    self, source=source, info=self.info, n_proc=n_proc
                )
                self.object_reader.start()
            ...
            if not self.info.cluster_info:
                cluster_info_file = os.path.join(
                    self.init.int_base, "cluster_info.pickle"
                )
            ...
            if not self.info.prime_info:
                ...
                if os.path.isfile(live_prime_phil):
                    ...
                    from prime.postrefine.mod_input import (
                        master_phil as prime_master_phil,
                    )
                    ...
                    live_prime_info_file = os.path.join(
                        self.init.int_base, "life_prime_info.pickle"
                    )
            ...
            if str(self.state).lower() == "finished" or len(
                self.info.finished_objects
            ) >= len(self.info.img_list):
                self.final_objects = [
                    i for i in self.info.finished_objects if not i.fail
                ]
