...
class TrackerPanel(wx.Panel):
    def __init__(self, parent):
        ...
        self.main_sizer.Add(
            self.info_panel, pos=(0, 0), span=(1, 2), flag=wx.EXPAND | wx.ALL, border=10
        )
...
class TrackerWindow(wx.Frame):
    ...
    def find_new_images(self):
        ...
        found_files = ginp.make_input_list(
            [self.data_folder], filter=True, filter_type="image", last=last_file
        )
    ...
    def plot_results(self):
        self.tracker_panel.chart.draw_plot(new_x=self.new_frames, new_y=self.new_counts)
        ...
        with Capturing() as junk_output:
            try:
                ...
                clusters, _ = ucs.ab_cluster(
                    5000, log=False, write_file_lists=False, schnell=False, doplot=False
                )
            except Exception, e:
                clusters = 0
