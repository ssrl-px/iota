...
class MiniButtonBoxInput(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        self.btn_mag = GradButton(
            self, bmp=viewmag_bmp, size=(31, 30), gradient_percent=0
        )
        ...
        self.btn_delete = GradButton(
            self, bmp=del_bmp, size=(31, 30), gradient_percent=0
        )
        ...
        self.btn_info = GradButton(
            self, bmp=info_bmp, size=(31, 30), gradient_percent=0
        )
...
class InputListCtrl(ulc.UltimateListCtrl, ListCtrlAutoWidthMixin):
    ...
    def __init__(
        ...
        ulc.UltimateListCtrl.__init__(self, parent, ID, pos, size=size, agwStyle=style)
...
class CtrlBase(wx.Panel):
    ...
    def __init__(
        self, parent, label_style="normal", content_style="normal", size=wx.DefaultSize
    ):
...
class CustomListCtrl(CtrlBase):
    def __init__(self, parent, size=wx.DefaultSize, content_style="normal"):
        CtrlBase.__init__(self, parent=parent, content_style=content_style, size=size)
    ...
    def onTypeChoice(self, e):
        ...
        for idx in range(self.ctr.GetItemCount()):
            if self.ctr.GetItemData(idx).type_selection != 0:
                data_items += 1
        self.main_window.set_tool_state(self.main_window.tb_btn_run, (data_items > 0))
        e.Skip()
...
class WidgetFactory(object):
    ...
    w_args = ["text", "path", "choice", "checkbox", "input_list"]

    w_kwargs = ["grid", "browse_btn", "mag_btn", "onChange", "onUpdate", "onToggle"]
    ...
    def make_widget(parent, object, label):
        ...
        if wtype == "path":  # Two styles only: single line w/ Browse, or input list
            ...
            widget = ChoiceCtrl(
                parent=parent, choices=["blah", "bleh", "pfui"], label=label
            )
