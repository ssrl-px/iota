...
def get_file_list(path, as_string=False, ignore_ext=None, ext_only=None):
    file_list = []
...
class GenerateInput:
    ...
    def test_file_list(self, filelist):
        ...
        for item in filelist:
            ...
            print 'File {0}, type "{1}"   .... {2} sec'.format(
                os.path.basename(item), file_type, run_time
            )
        print
        print "{0} FILES INSPECTED, TOTAL TIME: {1} sec" "".format(
            len(filelist), time.time() - all_start_time
        )
