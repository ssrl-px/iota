...
from iota.components.iota_utils import (
    UnicodeCharacters,
    WxFlags,
    noneset,
    norm_font_size,
...
class ImportWindow(BaseDialog):
    ...
    def __init__(
        ...
        self.img_triage = ct.ChoiceCtrl(
            self,
            label="Diffraction triage:",
            choices=["no_triage", "simple", "grid_search"],
            ctrl_size=(150, -1),
        )
        trg_box_sizer.Add(self.img_triage, flag=wx.ALL | wx.EXPAND, border=10)

        self.min_bragg_peaks = ct.OptionCtrl(
            self,
            items=[("n_bragg", 10)],
        ...
        self.triage_spot_height = ct.OptionCtrl(
            self,
            items=[("min", ""), ("max", "")],
        ...
        trg_box_sizer.Add(self.triage_spot_height, flag=wx.ALL | wx.EXPAND, border=10)

        self.triage_spot_area = ct.OptionCtrl(
            self,
            items=[("min", ""), ("max", "")],
        ...
        self.triage_step_size = ct.OptionCtrl(
            self,
            items=[("step", "")],
    ...
    def read_phil(self):
        ...
        custom_filename = str(
            self.params.cctbx_ha14.image_conversion.rename_pickle_prefix
        ).lower()
    ...
    def onOK(self, e):
        ...
        self.phil_text = "\n".join(
            [
                ...
                "{",
                "  type = {}".format(
                    self.img_triage.ctr.GetString(self.img_triage.ctr.GetSelection())
                ),
                "    .type = choice",
                "  min_Bragg_peaks = {}".format(
                    self.min_bragg_peaks.n_bragg.GetValue()
                ),
                "  grid_search",
                "  {",
...
class OldBackendOptions(BaseBackendDialog):
    ...
    def read_param_phil(self):
        ...
        self.signal_search.SetValue(
            self.params.cctbx_ha14.grid_search.sig_height_search
        )
        ...
        if self.params.cctbx_ha14.selection.prefilter.flag_on:
            ...
            if self.params.cctbx_ha14.selection.prefilter.target_unit_cell is not None:
                try:
                    uc = (
                        self.params.cctbx_ha14.selection.prefilter.target_unit_cell.parameters()
                    )
                except AttributeError:
                    uc = None
            else:
...
class BackendOptions(BaseBackendDialog):
    ...
    def __init__(self, parent, phil, target, *args, **kwargs):
        ...
        self.estimate_gain = wx.CheckBox(
            self.opt_options, label="Estimate gain for each image"
        )
