...
class ImageFinderThread(Thread):
    ...
    def __init__(self, parent, image_path, image_list):
        Thread.__init__(self)
        self.parent = parent
    ...
    def run(self):
        ...
        tested_file_list = [
            i for i in new_file_list if ginp.get_file_type(i) != "not image"
        ]

        # Generate list of new images
        new_img = [
            [i, len(ext_file_list) + 1, j]
            for i, j in enumerate(tested_file_list, len(old_file_list) + 1)
        ]
...
class ProcWindow(wx.Frame):
    ...
    def __init__(self, parent, id, title, params, target_phil=None, test=False):
        ...
        self.tb_btn_timeout = self.proc_toolbar.AddCheckLabelTool(
            wx.ID_ANY,
            label="Timeout",
...
class ImportWindow(wx.Dialog):
    ...
    def __init__(self, *args, **kwargs):
        ...
        vbox.Add((-1, 10))
        vbox.Add(trg_box, flag=wx.ALL | wx.EXPAND, border=10)
...
class IOTAPreferences(wx.Dialog):
    def __init__(self, *args, **kwargs):
        ...
        queue_sizer.Add(self.queues, flag=wx.LEFT | wx.RIGHT | wx.BOTTOM, border=10)
        ...
        queue_sizer.Add(
            self.custom_queue, flag=wx.LEFT | wx.RIGHT | wx.BOTTOM, border=10
        )
        ...
        self.opt_timeout = ct.OptionCtrl(
            self,
            items=[("timeout", "")],
            label="Timeout (sec):",
            label_size=(120, -1),
        ...
        proc_sizer.Add(
            self.chk_mm_timeout, flag=wx.LEFT | wx.RIGHT | wx.BOTTOM, border=10
        )
        proc_sizer.Add(self.opt_timeout, flag=wx.LEFT | wx.RIGHT | wx.BOTTOM, border=10)
    ...
    def set_choices(self, method, queue, monitor_mode, timeout, to_length):
