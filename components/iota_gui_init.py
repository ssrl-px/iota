...
class ProcThread(Thread):
    ...
    def __init__(self, parent, init, iterable, input_type="image"):
    ...
    def run(self):
        if self.init.params.mp_method == "multiprocessing":
            ...
            command = "bsub -q {} -n {} iota.process {} --files {} --type {}" "".format(
                queue, nproc, init_path, iter_path, self.type
            )
            print command
...
class MainWindow(wx.Frame):
    ...
    def __init__(self, parent, id, title):
        ...
        self.tb_btn_imglist = self.toolbar.AddLabelTool(
            wx.ID_ANY,
        ...
        self.tb_btn_convert = self.toolbar.AddLabelTool(
            wx.ID_ANY,
        ...
        self.toolbar.AddSeparator()
        self.tb_btn_prime = self.toolbar.AddLabelTool(
            wx.ID_ANY,
            label="PRIME",
        ...
        self.tb_btn_merge = self.toolbar.AddLabelTool(
            wx.ID_ANY,
            label="cxi.merge",
        ...
        # self.toolbar.AddSeparator()
        self.tb_btn_test = self.toolbar.AddLabelTool(
            wx.ID_ANY, label="Test", bitmap=wx.Bitmap("{}/32x32/test.png".format(icons))
        )
    ...
    def onPreferences(self, e):
        ...
        dlg.set_choices(method=self.gparams.mp_method, queue=self.gparams.mp_queue)
    ...
    def init_settings(self):
        ...
        self.gparams.advanced.integrate_with = str(
            self.input_window.int_box.ctr.GetString(int_index)[:5]
        ).lower()
        ...
        self.gparams.advanced.random_sample.flag_on = (
            self.input_window.opt_chk_random.GetValue()
        )
        self.gparams.advanced.random_sample.number = (
            self.input_window.opt_spc_random.GetValue()
        )
    ...
    def onRun(self, e):
        ...
        self.proc_window = ProcWindow(
            self, -1, title=title, target_phil=self.target_phil, params=self.gparams
        )
    ...
    def onLoadScript(self, e):
        ...
        if load_dlg.ShowModal() == wx.ID_OK:
            ...
            if (
                self.gparams.advanced.integrate_with == "cctbx"
                and str(self.gparams.cctbx.target).lower() != "none"
            ):
                with open(self.gparams.cctbx.target, "r") as tf:
                    self.input_window.target_phil = tf.read()
            elif (
                self.gparams.advanced.integrate_with == "dials"
                and str(self.gparams.dials.target).lower() != "none"
            ):
            ...
            self.input_window.opt_chk_random.SetValue(
                self.gparams.advanced.random_sample.flag_on
            )
            ...
            self.input_window.opt_spc_random.SetValue(
                int(self.gparams.advanced.random_sample.number)
            )
    ...
    def onWriteImageList(self, e):
        ...
        if img_list_dlg.ShowModal() == wx.ID_OK:
            ...
            if good_init:
                wx.MessageBox(
                    "List of images saved in {}".format(img_list_dlg.GetPath()),
                    "Info",
...
class SummaryTab(wx.Panel):
    def __init__(
        ...
        if self.gparams.advanced.integrate_with == "cctbx":
            ...
            self.int_heatmap = ct.GradButton(
                self,
                bmp=wx.Bitmap("{}/24x24/heatmap.png" "".format(icons)),
    ...
    def onPRIME(self, e):
        ...
        self.prime_window = PRIMEWindow(
            self, -1, title="PRIME", prefix=self.gparams.prime_prefix
        )
...
class ProcWindow(wx.Frame):
    ...
    def analyze_results(self):
        if len(self.final_objects) == 0:
            ...
            # Do analysis
            analysis = Analyzer(
                self.init, self.img_objects, iota_version, gui_mode=True
            )
            plot = Plotter(self.gparams, self.final_objects, self.init.viz_base)

            # Initialize summary tab
            prime_file = os.path.join(
                self.init.int_base, "{}.phil".format(self.gparams.prime_prefix)
            )
    ...
    def plot_integration(self):
        try:
            ...
            names_numbers.append(
                [
                    "not processed",
                    len(self.img_list) - sum([i[1] for i in names_numbers]),
                ]
            )
            ...
            nsref = self.chart_tab.nsref_axes.bar(
                nsref_x,
                nsref_y,
                align="center",
                linewidth=0,
            ...
            self.chart_tab.nsref_axes.set_xticks(
                np.arange(len(nsref_x)) + 0.5, minor=False
            )
            ...
            res_y = np.array([np.nan if i == 0 else i for i in self.res_list]).astype(
                np.double
            )
            res_m = np.isfinite(res_y)
            self.chart_tab.res_axes.plot(
                res_x[res_m], res_y[res_m], "deepskyblue", lw=3
            )
    ...
    def onTimer(self, e):
        ...
        if self.gparams.image_conversion.convert_only:
            ...
            processed_images = [
                i for i in self.objects_in_progress if i.final["final"] != None
            ]
            self.sb.SetStatusText(
    ...
    def finish_process(self):
        if os.path.isfile(self.tmp_abort_file):
            ...
            self.sb.SetStatusText(
                ...
                "".format(len(self.final_objects), len(self.img_objects)),
                1,
            )
            if len(self.final_objects) > 0:
                self.plot_integration()
...
class InputWindow(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        # Title box
        self.title_box = ct.InputCtrl(self, label="Title: ", label_size=(120, -1))
        vbox.Add(
    ...
    def onImportOptions(self, e):
        """On clicking the Import Options button opens the Import Options
        window."""
        imp_dialog = ImportWindow(
            self, title="Import Options", style=wx.DEFAULT_DIALOG_STYLE | wx.STAY_ON_TOP
        )
        imp_dialog.Fit()
        ...
        imp_dialog.conv_ctr_prefix.SetValue(
            str(self.gparams.image_conversion.rename_pickle_prefix)
        )
        ...
        imp_dialog.mod_ctr_beamstop.SetValue(
            str(self.gparams.image_conversion.beamstop)
        )
        imp_dialog.mod_ctr_dist.SetValue(str(self.gparams.image_conversion.distance))
        imp_dialog.mod_ctr_beamx.SetValue(
            str(self.gparams.image_conversion.beam_center.x)
        )
        imp_dialog.mod_ctr_beamy.SetValue(
            str(self.gparams.image_conversion.beam_center.y)
        )
        ...
        imp_dialog.trg_ctr_bragg.SetValue(
            str(self.gparams.image_triage.min_Bragg_peaks)
        )
        imp_dialog.trg_gs_hmin.SetValue(
            str(self.gparams.image_triage.grid_search.height_min)
        )
        imp_dialog.trg_gs_hmax.SetValue(
            str(self.gparams.image_triage.grid_search.height_max)
        )
        imp_dialog.trg_gs_amin.SetValue(
            str(self.gparams.image_triage.grid_search.area_min)
        )
        imp_dialog.trg_gs_amax.SetValue(
            str(self.gparams.image_triage.grid_search.area_max)
        )
        ...
        if imp_dialog.ShowModal() == wx.ID_OK:
            if imp_dialog.conv_rb1_prefix.GetValue():
                ...
                self.gparams.image_conversion.rename_pickle_prefix = (
                    imp_dialog.conv_ctr_prefix.GetValue()
                )
            ...
            self.gparams.image_conversion.beamstop = float(
                imp_dialog.mod_ctr_beamstop.GetValue()
            )
            self.gparams.image_conversion.distance = float(
                imp_dialog.mod_ctr_dist.GetValue()
            )
            self.gparams.image_conversion.beam_center.x = float(
                imp_dialog.mod_ctr_beamx.GetValue()
            )
            self.gparams.image_conversion.beam_center.y = float(
                imp_dialog.mod_ctr_beamy.GetValue()
            )
            ...
            self.gparams.image_triage.min_Bragg_peaks = int(
                imp_dialog.trg_ctr_bragg.GetValue()
            )
            self.gparams.image_triage.grid_search.area_min = int(
                imp_dialog.trg_gs_amin.GetValue()
            )
            self.gparams.image_triage.grid_search.area_max = int(
                imp_dialog.trg_gs_amax.GetValue()
            )
            self.gparams.image_triage.grid_search.height_min = int(
                imp_dialog.trg_gs_hmin.GetValue()
            )
            self.gparams.image_triage.grid_search.height_max = int(
                imp_dialog.trg_gs_hmax.GetValue()
            )
    ...
    def onProcessOptions(self, e):
        ...
        if self.int_box.ctr.GetCurrentSelection() == 0:
            ...
            int_dialog.gs_ctr_height.SetValue(
                str(self.gparams.cctbx.grid_search.height_median)
            )
            int_dialog.gs_ctr_hrange.SetValue(
                str(self.gparams.cctbx.grid_search.height_range)
            )
            int_dialog.gs_ctr_area.SetValue(
                str(self.gparams.cctbx.grid_search.area_median)
            )
            int_dialog.gs_ctr_arange.SetValue(
                str(self.gparams.cctbx.grid_search.area_range)
            )
            int_dialog.gs_chk_sih.SetValue(
                self.gparams.cctbx.grid_search.sig_height_search
            )
            int_dialog.sel_chk_only.SetValue(
                self.gparams.cctbx.selection.select_only.flag_on
            )
            int_dialog.onSelOnly(wx.EVT_CHECKBOX)
            if (
                str(self.gparams.cctbx.selection.select_only.grid_search_path).lower()
                == "none"
            ):
                int_dialog.sel_ctr_objpath.SetValue("")
            else:
                int_dialog.sel_ctr_objpath.SetValue(
                    str(self.gparams.cctbx.selection.select_only.grid_search_path)
                )
            int_dialog.sel_ctr_minsigma.SetValue(
                str(self.gparams.cctbx.selection.min_sigma)
            )
            ...
            if self.gparams.cctbx.selection.prefilter.flag_on:
                if (
                    ...
                    int_dialog.sel_ctr_lattice.SetValue(
                        str(self.gparams.cctbx.selection.prefilter.target_pointgroup)
                    )
                if (
                    str(self.gparams.cctbx.selection.prefilter.target_unit_cell).lower()
                    != "none"
                ):
                    t_uc = (
                        self.gparams.cctbx.selection.prefilter.target_unit_cell.parameters()
                    )
                    ...
                    int_dialog.sel_ctr_uc_tol.SetValue(
                        str(self.gparams.cctbx.selection.prefilter.target_uc_tolerance)
                    )
                if (
                    str(self.gparams.cctbx.selection.prefilter.min_reflections).lower()
                    != "none"
                ):
                    int_dialog.sel_chk_minref.SetValue(True)
                    int_dialog.sel_ctr_minref.Enable()
                    int_dialog.sel_ctr_minref.SetValue(
                        str(self.gparams.cctbx.selection.prefilter.min_reflections)
                    )
                if (
                    str(self.gparams.cctbx.selection.prefilter.min_resolution).lower()
                    != "none"
                ):
                    int_dialog.sel_chk_res.SetValue(True)
                    int_dialog.sel_ctr_res.Enable()
                    int_dialog.sel_ctr_res.SetValue(
                        str(self.gparams.cctbx.selection.prefilter.min_resolution)
                    )
            ...
            if int_dialog.ShowModal() == wx.ID_OK:
                ...
                if int_dialog.gs_ctr_area.GetValue() != "":
                    self.gparams.cctbx.grid_search.area_median = int(
                        int_dialog.gs_ctr_area.GetValue()
                    )
                if int_dialog.gs_ctr_arange.GetValue() != "":
                    self.gparams.cctbx.grid_search.area_range = int(
                        int_dialog.gs_ctr_arange.GetValue()
                    )
                if int_dialog.gs_ctr_height.GetValue() != "":
                    self.gparams.cctbx.grid_search.height_median = int(
                        int_dialog.gs_ctr_height.GetValue()
                    )
                if int_dialog.gs_ctr_hrange.GetValue() != "":
                    self.gparams.cctbx.grid_search.height_range = int(
                        int_dialog.gs_ctr_hrange.GetValue()
                    )
                self.gparams.cctbx.grid_search.sig_height_search = (
                    int_dialog.gs_chk_sih.GetValue()
                )
                self.gparams.cctbx.selection.select_only.flag_on = (
                    int_dialog.sel_chk_only.GetValue()
                )
                if int_dialog.sel_ctr_objpath.GetValue() == "":
                    self.gparams.cctbx.selection.select_only.grid_search_path = None
                else:
                    self.gparams.cctbx.selection.select_only.grid_search_path = (
                        int_dialog.sel_ctr_objpath.GetValue()
                    )
                if int_dialog.sel_ctr_minsigma.GetValue() != "":
                    self.gparams.cctbx.selection.min_sigma = float(
                        int_dialog.sel_ctr_minsigma.GetValue()
                    )
                ...
                if int_dialog.sel_chk_lattice.GetValue():
                    ...
                    self.gparams.cctbx.selection.prefilter.target_pointgroup = (
                        int_dialog.sel_ctr_lattice.GetValue()
                    )
                else:
                    self.gparams.cctbx.selection.prefilter.target_pointgroup = None
                if (
                    int_dialog.sel_chk_unitcell.GetValue()
                    and int_dialog.sel_ctr_uc_a.GetValue() != ""
                    and int_dialog.sel_ctr_uc_b.GetValue() != ""
                    and int_dialog.sel_ctr_uc_c.GetValue() != ""
                    and int_dialog.sel_ctr_uc_alpha.GetValue() != ""
                    and int_dialog.sel_ctr_uc_beta.GetValue() != ""
                    and int_dialog.sel_ctr_uc_gamma.GetValue() != ""
                ):
                    self.gparams.cctbx.selection.prefilter.flag_on = True
                    uc_tuple = (
                        float(int_dialog.sel_ctr_uc_a.GetValue()),
                    ...
                    self.gparams.cctbx.selection.prefilter.target_unit_cell = unit_cell(
                        uc_tuple
                    )
                    if int_dialog.sel_ctr_uc_tol.GetValue() != "":
                        self.gparams.cctbx.selection.prefilter.target_uc_tolerance = float(
                            int_dialog.sel_ctr_uc_tol.GetValue()
                        )
                    else:
                ...
                if int_dialog.sel_chk_minref.GetValue():
                    self.gparams.cctbx.selection.prefilter.flag_on = True
                    self.gparams.cctbx.selection.prefilter.min_reflections = int(
                        int_dialog.sel_ctr_minref.GetValue()
                    )
                else:
                    self.gparams.cctbx.selection.prefilter.min_reflections = None
                if int_dialog.sel_chk_res.GetValue():
                    self.gparams.cctbx.selection.prefilter.flag_on = True
                    self.gparams.cctbx.selection.prefilter.min_resolution = float(
                        int_dialog.sel_ctr_res.GetValue()
                    )
                else:
                    self.gparams.cctbx.selection.prefilter.min_resolution = None
                if (
                    int_dialog.sel_chk_lattice.GetValue() == False
                    and int_dialog.sel_chk_unitcell.GetValue() == False
                    and int_dialog.sel_chk_minref.GetValue() == False
                    and int_dialog.sel_chk_res.GetValue() == False
                ):
        ...
        # For DIALS options
            ...
            if int_dialog.ShowModal() == wx.ID_OK:
                ...
                if int_dialog.d_ctr_smin.GetValue() != "":
                    self.gparams.dials.min_spot_size = int(
                        int_dialog.d_ctr_smin.GetValue()
                    )
                if int_dialog.d_ctr_gthr.GetValue() != "":
                    self.gparams.dials.global_threshold = int(
                        int_dialog.d_ctr_gthr.GetValue()
                    )

        int_dialog.Destroy()

    def onAnalysisOptions(self, e):
        an_dialog = AnalysisWindow(
            self,
        ...
        if an_dialog.an_chk_cluster.GetValue():
            ...
            an_dialog.an_ctr_cluster.SetValue(
                str(self.gparams.analysis.cluster_threshold)
            )
        ...
        if an_dialog.ShowModal() == wx.ID_OK:
            ...
            self.gparams.analysis.cluster_threshold = float(
                an_dialog.an_ctr_cluster.GetValue()
            )
...
class CCTBXOptions(wx.Dialog):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.sel_rb1_selby = wx.RadioButton(
            self, label="Ewald proximal volume", style=wx.RB_GROUP
        )
        ...
        self.sel_txt_uc_alpha = wx.StaticText(
            self, label=u"\N{GREEK SMALL LETTER ALPHA} ="
        )
        self.sel_ctr_uc_alpha = wx.TextCtrl(self, size=(60, -1))
        self.sel_txt_uc_beta = wx.StaticText(
            self, label=u"\N{GREEK SMALL LETTER BETA} ="
        )
        self.sel_ctr_uc_beta = wx.TextCtrl(self, size=(60, -1))
        self.sel_txt_uc_gamma = wx.StaticText(
            self, label=u"\N{GREEK SMALL LETTER GAMMA} ="
        )
    ...
    def onSelOnlyBrowse(self, event):
        ...
        'Input' box w/ selection."""
        dlg = wx.DirDialog(
            self, "Choose directory with image objects:", style=wx.DD_DEFAULT_STYLE
        )
        if dlg.ShowModal() == wx.ID_OK:
...
class DIALSOptions(wx.Dialog):
    ...
    def onImportPHIL(self, e):
        dlg = wx.FileDialog(
            self,
            message="Select DIALS target file",
            defaultDir=os.curdir,
            defaultFile="*.phil",
            wildcard="*",
            style=wx.OPEN | wx.CHANGE_DIR,
...
class InitAll(object):
    ...
    def make_input_list(self):
        """Reads input directory or directory tree and makes lists of input
        images (in pickle format) using absolute path for each file.

        If a separate file with list of images is provided, parses that
        file and uses that as the input list. If random input option is
        selected, pulls a specified number of random images from the
        list and outputs that subset as the input list.
        """
        ...
        if (
            ...
            and self.params.advanced.random_sample.number < len(input_list)
        ):
    ...
    def make_int_object_list(self):
        ...
        if (
            self.params.advanced.random_sample.flag_on
            and self.params.advanced.random_sample.number < len(img_objects)
        ):
        ...
        if self.target_phil == None:
            if self.params.advanced.integrate_with == "cctbx":
                write_def = wx.MessageDialog(
                    None,
                    "WARNING! No target file for CCTBX.XFEL. " "Generate defaults?",
                    "WARNING",
                ...
                write_def = wx.MessageDialog(
                    None,
                    "WARNING! No target file for DIALS. " "Generate defaults?",
                    "WARNING",
                    wx.YES_NO | wx.NO_DEFAULT | wx.ICON_EXCLAMATION,
                )
                if write_def.ShowModal() == wx.ID_YES:
