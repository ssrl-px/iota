...
class IOTABaseFrame(wx.Frame):
    ...
    def onClose(self, e):
        """wx.EVT_CLOSE event handler.

        For subclass-specific stuff, override the self._clean_up()
        function
        """
    ...
    def _clean_up(self):
        """Clean-up procedure pre-exit.

        Override if there are any subclass-specific steps to be taken
        """
        pass
    ...
    def add_tool(
        ...
        if wx4:
            return self.toolbar.AddTool(
                toolId=id, label=label, kind=kind, bitmap=bmp, shortHelp=short_string
            )
        else:
            return self.toolbar.AddLabelTool(
                id=id,
                label=label,
                kind=kind,
                bitmap=bmp,
    ...
    def set_tool_states(self, tools):
        for tool in tools:
            self.set_tool_state(
                tool=tool[0], enable=tool[1], toggle=tool[2] if len(tool) == 3 else None
            )
...
class PHILPanelFactory(IOTABasePanel):
    ...
    def add_definition_control(self, parent, obj):
        ...
        sizer.Add(wdg, flag=wx.RIGHT | wx.LEFT | wx.BOTTOM | wx.EXPAND, border=5)
